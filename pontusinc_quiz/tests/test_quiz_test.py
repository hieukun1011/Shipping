...
class TestQuiz(TestQuizCommon):
    ...
    def test_case_4_quiz_2(self):
        quiz = self.env.ref('pontusinc_quiz.op_qz_2')
        if not quiz:
            raise AssertionError(
                'Error in data, please check for reference : '
                'pontusinc_quiz.op_qz_1')
